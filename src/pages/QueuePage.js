...
export function QueuePage({ patients, onStatusChange }) {
  ...
  const byDept = todayPts.reduce((acc, p) => {
    if (!acc[p.department]) acc[p.department] = [];
    acc[p.department].push(p);
    return acc;
  }, {});

  const totalWaiting = todayPts.filter(
      ...
      <div className="stats-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
        <div className="stat-card">
          <div className="stat-top">
            <span className="stat-lbl">Всего на сегодня</span>
            <div className="stat-ic" style={{ background: "#dbeafe" }}><Calendar size={18} color="#1d4ed8" /></div>
          </div>
        ...
        <div className="stat-card">
          <div className="stat-top">
            <span className="stat-lbl">Ожидают / На приёме</span>
            <div className="stat-ic" style={{ background: "#fde68a" }}><Clock size={18} color="#b45309" /></div>
          </div>
          <div className="stat-val">{totalWaiting}</div>
          <div className="stat-foot">человек в очереди</div>
        </div>
        <div className="stat-card">
          <div className="stat-top">
            <span className="stat-lbl">Завершено</span>
            <div className="stat-ic" style={{ background: "#dcfce7" }}><CheckCircle size={18} color="#15803d" /></div>
          </div>
          <div className="stat-val">{todayPts.filter((p) => p.status === "completed").length}</div>
          <div className="stat-foot">приёмов сегодня</div>
        </div>
      </div>

      {Object.keys(byDept).length === 0 ? (
        <div className="card" style={{ padding: "48px 0", textAlign: "center", color: "var(--muted)", fontSize: 14 }}>
          На сегодня записей нет
        </div>
      ) : (
        <div className="queue-grid">
          {Object.entries(byDept).map(([dept, pts]) => (
            <div className="queue-card" key={dept}>
