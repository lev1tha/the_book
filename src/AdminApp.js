...
export default function AdminApp() {
  ...
  // ─── Firebase: live-sync online bookings ─────────────────────────────────
  useEffect(() => {
    ...
    try {
      ...
      const unsub = onSnapshot(q, snap => {
        snap.docChanges().forEach(change => {
          ...
          const fbPatient = {
            id: "fb_" + change.doc.id,
            lastName: d.lastName||"", firstName: d.firstName||"", middleName: d.middleName||"",
            birthDate: d.birthDate||"", gender: d.gender||"unknown",
            phone: d.phone||"", email: d.email||"", iin: "", address: "",
            department: d.department||"", doctor: d.doctor||"",
            appointmentDate: d.appointmentDate||"", appointmentTime: d.appointmentTime||"",
            complaint: d.complaint||"", status: "confirmed", queueNum: 0, source: "online",
          };
          setPatients(prev => {
            if (prev.find(p => p.id === fbPatient.id)) return prev;
            return [fbPatient, ...prev];
          });
          setNotifs(prev => [{
            id: Date.now(), type:"new", read:false,
            text: `Онлайн-запись: ${fbPatient.lastName} ${fbPatient.firstName} → ${fbPatient.department}`,
          }, ...prev]);
        });
      });
      return unsub;
